import json, os, time, kokkoro
#from hoshino.typing import CQEvent
#from hoshino.config import NICKNAME
from kokkoro.service import Service
from kokkoro.common_interface import *
#allow the usage of 'EventInterface', supplimenting the original CQEvent
...
"""
def render_forward_msg(ev: EventInterface, msg_list: list): #dont think it is possible in discord?
                                                            #There is no native forwarding system in Discord.
    forward_msg = []
    for msg in msg_list:
        forward_msg.append({
            "type": "node",
...
async def create_room(bot, ev):
    global room
    args = ev.get_content().strip().split()
    try:
        game_name = args[1]
        room_num = str(args[2])
        member_limit = int(args[3])
        room_time = int(args[4]) if len(args) >= 5 else 30
    except:
        await bot.kkr_send(ev, '格式不对哦，@我发送开车帮助来看看怎么开车吧')
        return
    uid = str(ev.get_author_id())
    host_name = ev.get_author().get_nick_name()
    if len(host_name) == 0: #如果群友没有设置群昵称，则获取个人昵称
        host_name = ev.get_author_name()
    check_time()
    if uid in room:
        await bot.kkr_send(ev, '你已经发过车了哦')
        return
    if search_member(uid):
        await bot.kkr_send(ev, '你已经在别人车上了哦，不可以脚踏两条船哦')
        return
...
async def search_room(bot, ev):
    msg = ''
    check_time()
    uid = search_member(str(ev.get_author_id()))
    room_nums = len(room)
    if not room_nums:
        await bot.kkr_send(ev, '现在车站没有车哦！')
        return
    if uid:
...
async def join_room(bot, ev):
    check_time()
    mention_list = ev.get_mentions()
    if len(mention_list) > 1:
        uid = str(mention_list[-1].get_id())

    else:
        uid = ''
        m = ev.get_content().strip()
        for i in m:
            if i.isdigit():
               uid += i
    member_id = str(ev.get_author_id())
    member_name = ev.get_author().get_nick_name()
    if len(member_name) == 0:
        member_name = ev.get_author_name()
    new_member = {'member_name':member_name , 'member_id': member_id}
    if uid not in room:
        await bot.kkr_send(ev, '没有这辆车哦')
        return
    ...
    if member_limit:
        if len(room[uid]['member_list']) == member_limit:
            await bot.kkr_send(ev, '车队已经满员了哦')
            return
    ...
    if len(room[uid]['member_list']) == member_limit:
        ...
        await bot.kkr_send(ev, f'''{at_msg} 最后一位成员{member_name}({member_id})已上车，当前车上已满{member_limit}人''')
    else:
        await bot.kkr_send(ev, f'''{member_name}({member_id})已上车，当前车上已有{len(room[uid]['member_list'])}人''')
...
async def exit_room(bot, ev):
    ...
    if not uid:
        await bot.kkr_send(ev, '你没有在任何车队中哦')
    delete_member(uid, member_id)
    save_room()
    n = len(room[uid]['member_list'])
    if not n:
        close_room(uid)
        await bot.kkr_send(ev, f'''车队{uid}已解散''')
        return
    await bot.kkr_send(ev, f'''{member_name}({member_id})已下车，当前车{uid}上还有{n}人''')
...
async def master_call_member(bot, ev):
    ...
    if uid in room:
        ...
        for member in room[uid]['member_list']:
            at_member_id = member['member_id']
            at_msg += bot.kkr_at(at_member_id)
        await bot.kkr_send(ev, f'''到点了！{at_msg}''')
    else:
        await bot.kkr_send(ev, '你还没有开车哦')
